...
sort\_buffer\_size，就是MySQL为排序开辟的内存（sort\_buffer）的大小。**如果要排序的数据量小于sort\_buffer\_size，排序就在内存中完成。但如果排序数据量太大，内存放不下，则不得不利用磁盘临时文件辅助排序，采用归并排序。**
...
number\_of\_tmp\_files表示的是，排序过程中使用的临时文件数。你一定奇怪，为什么需要12个文件？内存放不下时，就需要使用外部排序，外部排序一般使用**归并排序**算法。可以这么简单理解，**MySQL将需要排序的数据分成12份，每一份单独排序后存在这些临时文件中。然后把这12个有序文件再合并成一个有序的大文件。**

如果**sort\_buffer\_size超过了需要排序的数据量的大小，number\_of\_tmp\_files就是0，**表示排序可以直接在内存中完成。
...
> tips：
>
> 普通排序，会遍历全表，将需要的值取出来，排序所需要的内存和参数 `sort_buffer_size`，进行比较
>
> * 如果 `sort_buffer_size` 值大于所需内存，那么就会在内存中进行排序，
> * 如果所需内存大于 `sort_buffer_size`，则会创建多个临时文件，进行归并排序合成一个大的有序文件。这里文件数与 `sort_buffer_size` 存在一个关系：`sort_buffer_size` 越小，需要分成的份数越多，`number_of_tmp_files` 的值就越大。
...
> rowid 排序与全字段排序的区别：
>
> * 全字段排序会直接将 `sort_buffer` 中的字段数据之间返回给结果。
> * `rowid` 排序会多一步，访问表 `id` 的索引取相关值进行返回。因为 `sort_buffer` 是没有完整的需要的数据结果。
>
> 结果集的描述：是一个逻辑概念，不是说，必须等到全部结果都查询到再返回，而是有一个就返回一个。
...
> tips：
>
> 后面两个索的例子说明你使用 order by 不是一定就会使用 sort_buffer 来完成排序。
>
> * 在无序情况，会申请空间进行排序（例如上面讲诉普通的order by）。
> * 如果存在索引，本身就是有序的，那么就会减少这个步骤，甚至使用联合索引可以直接取值。
